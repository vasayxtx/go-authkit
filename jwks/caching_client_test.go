...
func TestCachingClient_GetRSAPublicKey(t *testing.T) {
	...
	t.Run("ttl expiration", func(t *testing.T) {
		...
		// Third request after TTL expiration - should refetch from server
	...
	t.Run("ttl expiration with default value", func(t *testing.T) {
		jwksHandler := &idptest.JWKSHandler{}
		jwksServer := httptest.NewServer(jwksHandler)
		defer jwksServer.Close()
		issuerConfigHandler := &idptest.OpenIDConfigurationHandler{JWKSURL: jwksServer.URL}
		...
		// Create client without specifying TTL - should use default (1 hour)
		...
		// Second request immediately - should use cache (TTL not expired)
		pubKey2, err := cachingClient.GetRSAPublicKey(context.Background(), issuerConfigServer.URL, idptest.TestKeyID)
		require.NoError(t, err)
		require.NotNil(t, pubKey2)
		require.Equal(t, pubKey1, pubKey2)
	...
	t.Run("ttl expiration with concurrent requests", func(t *testing.T) {
		jwksHandler := &idptest.JWKSHandler{}
		jwksServer := httptest.NewServer(jwksHandler)
		defer jwksServer.Close()
		issuerConfigHandler := &idptest.OpenIDConfigurationHandler{JWKSURL: jwksServer.URL}
		...
		// Make multiple concurrent requests after TTL expiration
		var wg sync.WaitGroup
		const callsNum = 10
		wg.Add(callsNum)
		errs := make(chan error, callsNum)
		...
		// Should have refetched from server (initial request + one refetch after TTL expiration)
		require.EqualValues(t, 2, issuerConfigHandler.ServedCount())
		require.EqualValues(t, 2, jwksHandler.ServedCount())
	})

	t.Run("ttl with invalidate cache", func(t *testing.T) {
		jwksHandler := &idptest.JWKSHandler{}
		jwksServer := httptest.NewServer(jwksHandler)
		defer jwksServer.Close()
		issuerConfigHandler := &idptest.OpenIDConfigurationHandler{JWKSURL: jwksServer.URL}
		...
		// Manually invalidate cache after TTL expiration
		err = cachingClient.InvalidateCacheIfNeeded(context.Background(), issuerConfigServer.URL)
		require.NoError(t, err)
		require.EqualValues(t, 2, issuerConfigHandler.ServedCount())
		require.EqualValues(t, 2, jwksHandler.ServedCount())

		// Request after manual invalidation - should use the newly cached key
		pubKey2, err := cachingClient.GetRSAPublicKey(context.Background(), issuerConfigServer.URL, idptest.TestKeyID)
		require.NoError(t, err)
		require.NotNil(t, pubKey2)
		// Should still be 2 since we just invalidated
		require.EqualValues(t, 2, issuerConfigHandler.ServedCount())
		require.EqualValues(t, 2, jwksHandler.ServedCount())
	})
}
